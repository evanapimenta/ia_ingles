...
const diagnostic_prompt_1 = require("../prompts/diagnostic.prompt");
const vfu_prompt_1 = require("../prompts/vfu.prompt");
const classification_prompt_1 = require("../prompts/classification.prompt");
const teacher_report_prompt_1 = require("../prompts/teacher-report.prompt");
const suggested_answer_prompt_1 = require("../prompts/suggested-answer.prompt");
...
class EvaluationService {
    ...
    sessionService;
    llmProvider;
    constructor(preBlockService, sessionService) {
        ...
        if (isDemoMode) {
            ...
            this.llmProvider = new llm_service_1.MockProvider();
        }
        else {
            if (provider === 'openai') {
                if (hasOpenAI) {
                    console.log('EvaluationService: Using OpenAI Provider.');
                    this.llmProvider = new llm_service_1.OpenAIProvider();
                }
                else {
                ...
                if (hasGemini) {
                    ...
                    this.llmProvider = new llm_service_1.GeminiProvider();
                }
                else {
    ...
    async handleInitialSubmission(sessionId, artifact, narrative) {
        ...
        const filledPrompt = this.replacePlaceholders(diagnostic_prompt_1.diagnosticPrompt, {
            PRE_BLOCK: JSON.stringify(preBlock, null, 2),
            ARTIFACT: artifact,
            NARRATIVE: narrative
        });
        const hypothesis = await this.llmProvider.generateJSON(filledPrompt);
    ...
    async handleVFUAnswer(sessionId, answer) {
        const session = await this.sessionService.getSession(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
        ...
        session.updatedAt = new Date().toISOString();
        // Reanalysis Prompt
        const filledPrompt = this.replacePlaceholders(vfu_prompt_1.vfuPrompt, {
            PRE_BLOCK: JSON.stringify(preBlock, null, 2),
    ...
    async generateSuggestedAnswer(sessionId) {
        const session = await this.sessionService.getSession(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
        ...
        const filledPrompt = this.replacePlaceholders(suggested_answer_prompt_1.suggestedAnswerPrompt, {
            PRE_BLOCK: JSON.stringify(preBlock, null, 2),
            ARTIFACT: session.submission.artifact,
            NARRATIVE: session.submission.narrative,
            VFU_HISTORY: this.formatVFUHistory(session.vfuHistory),
        ...
        const result = await this.llmProvider.generateJSON(filledPrompt);
        return result.suggestedAnswer;
    }
    async generateFinalReport(session, preBlock) {
        ...
        const filledClassificationPrompt = this.replacePlaceholders(classification_prompt_1.classificationPrompt, {
            PRE_BLOCK: JSON.stringify(preBlock, null, 2),
            ARTIFACT: session.submission.artifact,
            NARRATIVE: session.submission.narrative,
            VFU_HISTORY: this.formatVFUHistory(session.vfuHistory),
        ...
        const classResult = await this.llmProvider.generateJSON(filledClassificationPrompt);
        // 2. Teacher Report Rationale Step
        const filledReportPrompt = this.replacePlaceholders(teacher_report_prompt_1.teacherReportPrompt, {
            PRE_BLOCK: JSON.stringify(preBlock, null, 2),
            ARTIFACT: session.submission.artifact,
            NARRATIVE: session.submission.narrative,
            VFU_HISTORY: this.formatVFUHistory(session.vfuHistory),
        ...
        const finalReport = await this.llmProvider.generateJSON(filledReportPrompt);
